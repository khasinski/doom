  ...
  module Render
    ...
    class Renderer
      ...
      def render_seg(seg)
        ...
        # Segment going right to left in screen space = backface
        return if sx1 >= sx2
      ...
      def draw_seg_range(x1, x2, sx1, sx2, dist1, dist2, sector, back_sector, sidedef, linedef)
        (x1..x2).each do |x|
          ...
          if back_sector
            ...
            # Draw ceiling
            if ceil_y < [front_ceil_y, back_ceil_y].max
              draw_flat_column(x, ceil_y, [front_ceil_y, back_ceil_y].max - 1, sector.ceiling_texture, sector.light_level, true)
            end

            # Draw floor
            if floor_y > [front_floor_y, back_floor_y].min
              draw_flat_column(x, [front_floor_y, back_floor_y].min + 1, floor_y, sector.floor_texture, sector.light_level, false)
            end

            # Upper wall
            if sector.ceiling_height > back_sector.ceiling_height && back_ceil_y > ceil_y
              draw_wall_column(x, ceil_y, back_ceil_y - 1, sidedef.upper_texture, dist, sector.light_level)
            end

            # Lower wall
            if sector.floor_height < back_sector.floor_height && back_floor_y < floor_y
            ...
            # Draw ceiling
            draw_flat_column(x, ceil_y, front_ceil_y - 1, sector.ceiling_texture, sector.light_level, true)

            # Draw floor
            draw_flat_column(x, front_floor_y + 1, floor_y, sector.floor_texture, sector.light_level, false)

            # Draw wall
            draw_wall_column(x, [ceil_y, front_ceil_y].max, [floor_y, front_floor_y].min, sidedef.middle_texture, dist, sector.light_level)
